...
#E. Pie Chart of credit card transactions
labels <- c("legit", "fraud")
labels <- paste(labels, round(100*prop.table(table(creditcardFraud$class)), 2))
labels <- paste(labels, "%")

pie(table(creditcardFraud$class), labels, col = c("orange", "red"),
    main = "Pie chart of credit card transactions")

#F. Compute the correlations among the variables
...
ggplot(aes(x=reorder(Model,-Recall) , y=Recall), data=model_compare_recall) +
  ...
  xlab('Models')  +
  ylab('Recall Measure')+
  geom_text(aes(label=round(Recall,4)))+
...
ggplot(aes(x=reorder(Model,-Precision) , y=Precision), data=model_compare_precision) +
  ...
  geom_text(aes(label=round(Precision,4)))+
  theme(axis.text.x = element_text(angle = 40))
...
ggplot(aes(x=reorder(Model,-F1) , y=F1), data=model_compare_f1) +
  ...
  xlab('Models')  +
  ylab('F1 Measure')+
  geom_text(aes(label=round(F1,4)))+
  theme(axis.text.x = element_text(angle = 40))
```
